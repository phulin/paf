  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        setSelected(
          (current) =>
            next.declarations.find((item) => item.id === current?.id) ??
            next.declarations[0] ??
            current,
        );
      } catch {
        if (!cancelled) {
          const filtered = demoStatementResponse.declarations.filter(
            (declaration) =>
              (!query ||
                `${declaration.name} ${declaration.signature} ${declaration.doc}`
                  .toLowerCase()
    ...
    <main className="main statement-page">
      <div className="statement-heading">
        ...
        <div className="statement-title-block">
          <div>
            <span className="eyebrow">Repository index</span>
            <h1>Lean statement browser</h1>
          </div>
          <div className="index-status">
            <Database size={14} />
            <span>{connected ? "filesystem index" : "demo index"}</span>
            <strong>
              {formatNumber(data.facets.statuses.proved + data.facets.statuses.sorry)} declarations
            </strong>
          </div>
        </div>
        <div className="statement-toolbar">
          <label className="statement-search">
            ...
            <input
              ...
              placeholder="Search names, types, docs…"
            />
            {query && (
...
function LibraryTree({
  ...
  return (
    <aside className="library-tree">
      ...
      <button
        className={`tree-row all ${book === "all" ? "active" : ""}`}
        onClick={() => setBook("all")}
      >
        <span>
          <HardDrive size={14} /> All books
        </span>
      ...
      <div className="book-list">
        {data.facets.books.map((item) => (
          <button
            className={`tree-row ${book === item.id ? "active" : ""}`}
            key={item.id}
            onClick={() => setBook(item.id)}
            title={item.label}
          >
            <span>
              <ChevronRight size={13} /> <b>{String(item.number).padStart(3, "0")}</b> {item.label}
            </span>
      ...
      <div className="tree-proof-summary">
        ...
        <div>
          ...
          <strong>{formatNumber(data.facets.statuses.proved)}</strong>
        </div>
        <div>
          <span>
            <CircleDashed size={13} /> sorry
          </span>
          <strong>{formatNumber(data.facets.statuses.sorry)}</strong>
        </div>
        <ProgressBar
          value={(100 * data.facets.statuses.proved) / Math.max(1, total)}
          color="var(--green)"
        />
      </div>
    </aside>
...
function ResultList({
  ...
  book: string;
  loading: boolean;
  selected: LeanStatement;
  setSelected: (item: LeanStatement) => void;
}) {
  return (
    <section className="result-list-panel">
      <div className="browser-panel-title results-title">
        ...
        <span>
          <Filter size={13} />{" "}
          {book === "all" ? "all books" : `book ${String(book).padStart(3, "0")}`}
        </span>
      </div>
      <div className={`result-list ${loading ? "loading" : ""}`}>
        {data.declarations.map((statement) => (
          <button
            key={statement.id}
            className={`statement-result ${selected?.id === statement.id ? "active" : ""}`}
            onClick={() => setSelected(statement)}
          >
            <div className={`declaration-icon kind-${statement.kind}`}>
              <KindIcon kind={statement.kind} />
            </div>
            <div className="statement-result-body">
              <div>
                <strong>{statement.name}</strong>
                <span className={`proof-dot ${statement.status}`} title={statement.status} />
              </div>
              <code>
                {statement.signature.split("\n").slice(1, 3).join(" ") || statement.signature}
              </code>
              <span>
                Book {String(statement.bookNumber).padStart(3, "0")} · Ch {statement.chapter} · §{" "}
                {statement.section}
              </span>
            </div>
          </button>
        ...
        {!data.declarations.length && (
          <div className="no-results">
            <Search size={24} />
            <strong>No declarations found</strong>
            <span>Try a broader name, kind, or proof filter.</span>
          </div>
        )}
      </div>
      {data.total > data.declarations.length && (
        <div className="result-limit">
          First {data.declarations.length} shown · refine your search
        </div>
      )}
    </section>
  );
    ...
    <section className="statement-detail">
      {selected ? (
        <>
          <div className="detail-header">
            <div className="detail-kind">
              <KindIcon kind={selected.kind} size={17} />
          ...
          <div className="statement-meta">
            <div>
              ...
              <strong>
                Book {String(selected.bookNumber).padStart(3, "0")} / Chapter {selected.chapter}
              </strong>
              <p>
                {selected.book} · § {selected.section}
              </p>
            </div>
            <div>
              ...
              <p>
                lines {selected.line}–{selected.endLine}
              </p>
            </div>
            <div>
              <span className="eyebrow">Verification</span>
              <strong className={selected.status === "proved" ? "success-text" : "warning-text"}>
                {selected.status === "proved" ? "closed term" : "open proof"}
              </strong>
              <p>
                {selected.status === "proved" ? "no sorry in declaration" : "proof contains sorry"}
              </p>
            </div>
          </div>
        </>
        ...
        <div className="detail-empty">
          <Code2 size={28} />
          <span>Select a declaration to inspect its source.</span>
        </div>
      )}
    </section>
  );
