          ...
          return [
            {
              location:
                typeof issue.location === "string"
                  ? issue.location
                  : "Source location not supplied",
              sourceExcerpt: typeof issue.source_excerpt === "string" ? issue.source_excerpt : "",
              description: issue.description,
              suggestedCorrection:
                typeof issue.suggested_correction === "string" ? issue.suggested_correction : "",
            },
          ];
        })
      : [];
    ...
    return (
      <div className="agent-report plain">
        <p className="agent-summary">
          {value || activity?.current || "No agent update recorded for this chapter."}
        </p>
      </div>
    );
  const findingCount =
    ...
    <div className="agent-report">
      ...
      {findingCount === 0 && (
        <div className="report-clear">
          ...
          <span>No issues or follow-up findings reported</span>
        </div>
      )}
      ...
      {report.sourceIssues.length > 0 && (
        <div className="report-group source-issues">
          <div className="report-group-title">
            ...
            <em>{report.sourceIssues.length}</em>
          </div>
...
export function AgentPlan({ activity }: { activity?: AgentActivity }) {
  ...
  const progress =
    total > 0
      ? Math.min(100, Math.round(((activity?.todo_completed ?? completed) / total) * 100))
      : 0;
  ...
  return (
    <div className="agent-plan">
      <div className="agent-plan-heading">
        ...
        {total > 0 && (
          <em>
            {activity?.todo_completed ?? completed}/{total}
          </em>
        )}
      </div>
      {todos.length > 0 ? (
        <>
          ...
          <ol>
            {todos.map((todo, index) => (
              <li
                className={`${todo.completed ? "completed" : ""} ${index === currentIndex ? "current" : ""}`}
                key={`${todo.text}-${index}`}
              >
