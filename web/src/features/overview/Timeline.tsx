      ...
      <div className="feed-list">
        {events.length ? (
          ...
          <div className="feed-empty">
            <Activity size={24} />
            <strong>No agent events right now</strong>
            <span>The stream will update when a run starts.</span>
          </div>
        )}
      </div>
    </section>
...
export function BuildPanel({
  ...
  const currentWorkUnitId = build.current_work_unit_id ?? build.current_chapter_id;
  ...
  return (
    <section className="panel build-panel">
      <div className="build-heading">
        ...
        <span className={`build-state ${build.active ? "active" : ""}`}>
          {build.active ? "building" : "idle"}
        </span>
      </div>
      <div className="build-target">
        ...
        <div>
          ...
          <strong>{currentWorkUnitId ?? "No work unit reserved"}</strong>
        </div>
      </div>
      <div className="build-progress-block">
        <div className="build-progress-label">
          <span>
            {build.completed ?? 0} / {build.total ?? 0} modules
          </span>
        ...
        <div className="build-counters">
          <span>
            <Check size={13} /> iter {build.iteration ?? 0}/{build.maximum_iterations ?? 0}
          </span>
          <span className={build.error_count ? "error-text" : ""}>
            <X size={13} /> {build.error_count ?? 0} errors
          </span>
          <span className={build.warning_count ? "warning-text" : ""}>
            <AlertTriangle size={13} /> {build.warning_count ?? 0} warnings
          </span>
        </div>
      </div>
      <div className="terminal-output">
        ...
        {(build.output_tail?.length
          ? build.output_tail
          : ["coordinator build idle", "waiting for next certified change…"]
        )
          .slice(-2)
          .map((line, index) => (
            <div key={index}>
      ...
      <button className="timeline-trigger" onClick={openTimeline}>
        <span className="timeline-trigger-icon">
          <Activity size={17} />
        </span>
        <span>
...
export function TimelineDrawer({ state, close }: { state: SwarmState; close: () => void }) {
  ...
  return (
    <div
      className="drawer-backdrop timeline-backdrop"
    ...
    >
      <aside className="timeline-drawer">
        ...
        <div className="timeline-drawer-summary">
          <span>
            <Users size={14} />
            <strong>{state.agents?.active ?? 0}</strong> agents
          </span>
          <span>
            <Cpu size={14} />
            <strong>{Object.keys(state.activities ?? {}).length}</strong> runs
          </span>
          <span>
            <Clock3 size={14} /> updated {timeAgo(state.updated_at)}
          </span>
        </div>
