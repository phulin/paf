...
export function TaskTable({
  ...
  const buildTargets = new Set(
    build.target_work_unit_ids?.length
      ? build.target_work_unit_ids
      ...
      <div className="table-scroll">
        ...
        {!visible.length && (
          <div className="empty-table">
            <ListFilter size={22} />
            <span>No chapters match this view.</span>
          </div>
        )}
      </div>
      <div className="table-footer">
        <span>
          Showing {Math.min(visible.length, 22)} of {visible.length} chapters
        </span>
