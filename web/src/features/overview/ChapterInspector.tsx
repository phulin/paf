...
function AgentTimelinePane({ activity }: { activity?: AgentActivity }) {
  ...
  const latestMessage = events.reduce<number | undefined>(
    (latest, event) =>
      event.kind === "message" && event.sequence !== undefined
    ...
    <section className="agent-timeline-pane">
      ...
      <div className="agent-timeline-meta">
        <span>
          ...
          {activity?.run_id?.slice(0, 12) ?? "no run"}
        </span>
        <span>
          <Activity size={13} />
      ...
      <div className="agent-event-list">
        {events.length > 0 ? (
          events.map((event, index) => {
            const detail =
              event.kind === "message" && event.sequence === latestMessage
                ? activity?.latest_summary || event.detail
                : event.detail;
            const report = event.kind === "message" ? parseAgentReport(detail) : null;
            return (
              <div
                className={`agent-event status-${event.status}`}
                key={event.sequence ?? `${event.at}-${index}`}
              >
                <time>{eventTime(event)}</time>
                <div className="agent-event-track">
                  <span />
                  <i />
                </div>
      ...
      <aside className="inspector-drawer">
        <div className="agent-detail-pane">
              ...
              return (
                <div className="drawer-stage" key={stage}>
                  ...
                  <div>
                    <strong>{stage}</strong>
                    <span>
                      {compactTaskDetail(task?.detail) ||
                        (task?.status === "succeeded"
                          ? `completed in ${task.rounds || 1} round`
                          : "not started")}
                    </span>
                  </div>
                  <StatusPill
          ...
          <div className="drawer-section">
            ...
            {activity && (
              <div className="agent-stats">
                <span>
                  <TerminalSquare size={14} /> {activity.commands ?? 0} shell
                </span>
                <span>
                  <Cpu size={14} /> {activity.mcp_calls ?? 0} MCP
                </span>
                <span>
                  <FileCode2 size={14} /> {activity.file_changes ?? 0} edits
                </span>
                <span>
                  <XCircle size={14} /> {activity.failures ?? 0} failures
                </span>
              </div>
            )}
          </div>
          <div className="drawer-section">
            ...
            <code>
              lean/LastLib/{row.book.replace("book", "Book")}/Chapter
              {String(row.number).padStart(2, "0")}/**/*.lean
            </code>
          </div>
        </div>
