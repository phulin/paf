...
export default function App() {
  const [view, setView] = useState<View>(
    window.location.hash === "#statements" ? "statements" : "overview",
  );
  const [live, setLive] = useState(true);
  const { state, swarms, selectedSwarm, selectSwarm, connected, fetching, systemLoad, refresh } =
    useSwarmState(live);
  ...
  useEffect(() => {
    const restoreView = () =>
      setView(window.location.hash === "#statements" ? "statements" : "overview");
