...
function compactTaskDetail(detail?: string): string {
...
function AgentTimelinePane({ activity }: { activity?: AgentActivity }) {
  ...
  const latestMessage = events.reduce<number | undefined>(
    (latest, event) => event.kind === "message" && event.sequence !== undefined
      ? Math.max(latest ?? event.sequence, event.sequence)
      : latest,
    undefined,
  );
  ...
  return (
    <section className="agent-timeline-pane">
      <div className="agent-timeline-header">
        <div><span className="eyebrow">Selected run</span><h2>Agent timeline</h2></div>
        <span className="tiny-live"><span /> tailing</span>
      </div>
      <div className="agent-timeline-meta">
        <span><TerminalSquare size={13} />{activity?.run_id?.slice(0, 12) ?? "no run"}</span>
        <span><Activity size={13} />{events.length} events</span>
        <span><Clock3 size={13} />{timeAgo(activity?.updated_at)}</span>
      </div>
      <div className="agent-event-list">
        {events.length > 0 ? events.map((event, index) => {
          const detail = event.kind === "message" && event.sequence === latestMessage
            ? activity?.latest_summary || event.detail
            : event.detail;
          ...
          return (
            <div className={`agent-event status-${event.status}`} key={event.sequence ?? `${event.at}-${index}`}>
              <time>{eventTime(event)}</time>
              <div className="agent-event-track"><span /><i /></div>
              <div className="agent-event-content">
                <div><EventKind kind={event.kind} /><em>{event.status}</em></div>
                <strong>{event.title}</strong>
                {(report?.summary || detail) && <p>{report?.summary || detail}</p>}
              </div>
            </div>
          );
        }) : (
          <div className="agent-timeline-empty">
            <Activity size={24} />
            <strong>No events recorded for this run</strong>
            <span>The timeline will update when the agent emits activity.</span>
          </div>
        )}
      </div>
    </section>
            ...
            {STAGES.map((stage, index) => {
              ...
              return (
                <div className="drawer-stage" key={stage}>
                  ...
                  <div><strong>{stage}</strong><span>{compactTaskDetail(task?.detail) || (task?.status === "succeeded" ? `completed in ${task.rounds || 1} round` : "not started")}</span></div>
